...
func init() {
	...
	statusCmd.PersistentFlags().StringVarP(&connectionTypeFilter, "filter-by-connection-type", "T", "", "filters the detailed output by connection type (P2P|Relayed|DERP), e.g., --filter-by-connection-type P2P")
...
func parseFilters() error {
	...
	case "", "p2p", "relayed", "derp":
		if strings.ToLower(connectionTypeFilter) != "" {
			enableDetailFlagWhenFilterFlag()
		}
	default:
		return fmt.Errorf("wrong connection-type filter, should be one of P2P|Relayed|DERP, got: %s", connectionTypeFilter)
	}
